...
from .generators.fanvil import generate_fanvil_phonebook
...
def fanvil_phonebook_xml(request):
    content = generate_fanvil_phonebook()
...
def dynamic_phonebook_xml(request, manufacturer, token):
    entry = get_object_or_404(
        PhonebookXML,
        manufacturer__iexact=manufacturer,
        token=token,
    ...
    if entry.manufacturer.lower() != "fanvil":
        return HttpResponse(
